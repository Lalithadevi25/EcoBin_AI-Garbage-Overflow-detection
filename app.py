import streamlit as st
from ultralytics import YOLO
from PIL import Image
import numpy as np
import cv2
import tempfile
import os
import smtplib
import subprocess
import imageio_ffmpeg
import requests

from io import BytesIO
from email.message import EmailMessage
from datetime import datetime
from zoneinfo import ZoneInfo
from streamlit_geolocation import streamlit_geolocation


# ============================================================
# PAGE CONFIG
# ============================================================

st.set_page_config(
    page_title="EcoBin AI",
    page_icon="♻️",
    layout="wide",
    initial_sidebar_state="collapsed"
)


# ============================================================
# SESSION STATE
# ============================================================

defaults = {
    "page": "home",
    "last_alert_time": None,

    "image_result": None,
    "image_file_name": None,

    "camera_result": None,

    "video_result": None,
    "video_file_name": None,

    "live_latitude": None,
    "live_longitude": None,
    "live_accuracy": None,
    "live_area": None,
    "location_loaded": False,
}

for key, value in defaults.items():
    if key not in st.session_state:
        st.session_state[key] = value


# ============================================================
# PREMIUM UI
# ============================================================

st.markdown(
    """
<style>

/* ==========================================================
   GLOBAL
   ========================================================== */

:root {
    --navy: #173b6c;
    --navy2: #0f2f59;
    --blue: #2f80ed;
    --green: #16a34a;
    --green2: #22c55e;
    --purple: #6d4aff;
    --text: #24344d;
    --muted: #64748b;
    --line: #d7e2ef;
    --bg: #f5f8fc;
    --white: #ffffff;
    --danger: #dc2626;
}

.stApp {
    background:
        radial-gradient(
            circle at 8% 5%,
            rgba(47,128,237,.08),
            transparent 25%
        ),
        radial-gradient(
            circle at 92% 8%,
            rgba(34,197,94,.07),
            transparent 25%
        ),
        #f5f8fc;
}

.block-container {
    max-width: 1280px;
    padding: 28px 5% 35px 5%;
}

#MainMenu,
footer,
header {
    visibility: hidden;
}


/* ==========================================================
   GENERAL TEXT
   ========================================================== */

.stMarkdown,
.stMarkdown p,
.stMarkdown li {
    color: var(--text);
}


/* ==========================================================
   HOME PAGE
   ========================================================== */

.hero-title {
    text-align: center;
    color: var(--navy);
    font-size: 31px;
    font-weight: 900;
    letter-spacing: -.5px;
    margin: 0;
}

.hero-subtitle {
    text-align: center;
    color: #52627a;
    font-size: 13px;
    margin: 5px 0 24px 0;
}

.hero-card {
    min-height: 300px;
    background: rgba(255,255,255,.97);
    border: 1px solid var(--line);
    border-radius: 20px;
    padding: 28px;
    box-shadow: 0 10px 30px rgba(23,59,108,.08);
    position: relative;
    overflow: hidden;
}

.hero-card:before {
    content: "";
    position: absolute;
    left: 0;
    top: 0;
    width: 100%;
    height: 5px;
    background: linear-gradient(
        90deg,
        #16a34a,
        #2f80ed,
        #6d4aff
    );
}

.hero-left {
    display: flex;
    flex-direction: column;
    justify-content: center;
    height: 100%;
}

.program-pill {
    display: inline-block;
    width: fit-content;
    padding: 7px 13px;
    border-radius: 999px;
    background: #eef5ff;
    color: var(--navy);
    border: 1px solid #cfe0fa;
    font-size: 11px;
    font-weight: 800;
    margin-bottom: 16px;
}

.hero-career {
    color: var(--navy);
    font-size: 27px;
    font-weight: 900;
    line-height: 1.18;
    margin: 0 0 8px 0;
}

.hero-capstone {
    color: #52627a;
    font-size: 14px;
    font-weight: 700;
    margin-bottom: 20px;
}

.hero-project-name {
    color: var(--green);
    font-size: 15px;
    font-weight: 800;
    margin-top: 10px;
}

.hero-logo {
    display: flex;
    align-items: center;
    gap: 13px;
    margin-top: 4px;
}

.hero-logo img {
    width: 72px;
    height: 72px;
    object-fit: contain;
    border-radius: 14px;
    background: #f8fafc;
    border: 1px solid #e2e8f0;
}

.description-card {
    min-height: 300px;
    background: #fff;
    border: 1px solid var(--line);
    border-radius: 20px;
    padding: 25px 27px;
    box-shadow: 0 10px 30px rgba(23,59,108,.07);
}

.description-heading {
    color: var(--navy);
    font-size: 18px;
    font-weight: 900;
    margin-bottom: 13px;
}

.description-card p {
    color: #52627a;
    font-size: 13px;
    line-height: 1.75;
    margin: 0 0 14px 0;
}

.section-title {
    color: var(--navy);
    font-size: 19px;
    font-weight: 900;
    margin: 28px 0 12px 0;
}

.capability-card {
    background: #fff;
    border: 1px solid var(--line);
    border-radius: 16px;
    padding: 18px 14px;
    min-height: 115px;
    text-align: center;
    box-shadow: 0 7px 20px rgba(23,59,108,.06);
}

.capability-icon {
    font-size: 21px;
    margin-bottom: 5px;
}

.capability-title {
    color: var(--navy);
    font-size: 13px;
    font-weight: 900;
}

.capability-text {
    color: #64748b;
    font-size: 10.5px;
    line-height: 1.45;
    margin-top: 5px;
}

.team-card {
    background: #fff;
    border: 1px solid var(--line);
    border-radius: 17px;
    padding: 19px;
    min-height: 150px;
    box-shadow: 0 7px 20px rgba(23,59,108,.05);
}

.team-heading {
    color: var(--navy);
    font-size: 11px;
    font-weight: 900;
    letter-spacing: .3px;
    margin-bottom: 10px;
}

.team-text {
    color: #52627a;
    font-size: 11px;
    line-height: 1.9;
}

.team-text a {
    color: #2563eb;
}

.footer-text {
    text-align: center;
    color: #64748b;
    font-size: 11px;
    margin-top: 25px;
}


/* ==========================================================
   PREDICTION PAGE HEADER
   ========================================================== */

.page-header {
    background: linear-gradient(
        135deg,
        #173b6c,
        #24548d
    );
    border-radius: 20px;
    padding: 22px 25px;
    color: white;
    margin-bottom: 18px;
    box-shadow: 0 12px 28px rgba(23,59,108,.16);
}

.page-header-title {
    font-size: 27px;
    font-weight: 900;
    margin: 0;
}

.page-header-sub {
    color: #e0ecff;
    font-size: 12px;
    margin-top: 5px;
}


/* ==========================================================
   LOCATION
   ========================================================== */

.location-card {
    background: linear-gradient(
        135deg,
        #eff8ff,
        #f0fdf4
    );
    border: 1px solid #b9d9f7;
    border-radius: 16px;
    padding: 15px 18px;
    margin: 8px 0 20px 0;
}

.location-heading {
    color: var(--navy);
    font-size: 15px;
    font-weight: 900;
    margin-bottom: 9px;
}

.location-line {
    color: #334155;
    font-size: 12px;
    line-height: 1.8;
}


/* ==========================================================
   SECTION TITLES
   ========================================================== */

.mode-title {
    color: var(--navy);
    font-size: 20px;
    font-weight: 900;
    margin: 25px 0 13px 0;
}


/* ==========================================================
   IMAGE / CAMERA / VIDEO CARDS
   ========================================================== */

.io-card {
    background: #ffffff;
    border: 1px solid #d5e1ee;
    border-radius: 18px;
    padding: 14px;
    min-height: 300px;
    box-shadow: 0 8px 22px rgba(23,59,108,.07);
    overflow: hidden;
}

.io-card-title {
    color: #173b6c;
    font-size: 15px;
    font-weight: 900;
    margin-bottom: 7px;
}

.io-card-subtitle {
    color: #52627a;
    font-size: 11px;
    font-weight: 600;
    margin-bottom: 9px;
}

.empty-message {
    min-height: 220px;
    display: flex;
    align-items: center;
    justify-content: center;
    text-align: center;

    color: #475569;

    background: linear-gradient(
        135deg,
        #f8fbff,
        #f1f6fb
    );

    border: 1px dashed #b9c9dc;
    border-radius: 13px;

    padding: 20px;

    font-size: 12px;
    font-weight: 700;

    line-height: 1.7;
}

.empty-icon {
    font-size: 28px;
    margin-bottom: 7px;
}

.empty-heading {
    color: #173b6c;
    font-size: 13px;
    font-weight: 900;
}

.empty-sub {
    color: #64748b;
    font-size: 11px;
    margin-top: 4px;
}


/* ==========================================================
   FILE UPLOADER
   ========================================================== */

div[data-testid="stFileUploader"] {
    color: #24344d !important;
}

div[data-testid="stFileUploader"] label {
    color: #334155 !important;
    font-weight: 800 !important;
}

div[data-testid="stFileUploaderDropzone"] {
    background: #f8fafc !important;
    border: 1px dashed #aebfd3 !important;
    border-radius: 12px !important;
}

div[data-testid="stFileUploaderDropzone"] * {
    color: #334155 !important;
}

div[data-testid="stFileUploaderDropzone"] button {
    background: #ffffff !important;
    color: #173b6c !important;
    border: 1px solid #9fb3ca !important;
}


/* ==========================================================
   BUTTONS
   ========================================================== */

div.stButton > button {
    width: 100%;
    border-radius: 10px !important;
    min-height: 44px;

    font-weight: 900 !important;

    border: 1px solid #c5d3e2 !important;

    color: #173b6c !important;
    background: #ffffff !important;

    transition: .2s;
}

div.stButton > button:hover {
    border-color: #2f80ed !important;
    color: #1d4ed8 !important;

    box-shadow:
        0 5px 15px rgba(47,128,237,.15);
}


/* Detection buttons */

.detect-button div.stButton > button {
    background: linear-gradient(
        135deg,
        #173b6c,
        #24548d
    ) !important;

    color: #ffffff !important;

    border: none !important;

    min-height: 46px;

    font-size: 14px !important;

    box-shadow:
        0 7px 17px rgba(23,59,108,.18);
}

.detect-button div.stButton > button:hover {
    color: #ffffff !important;

    background: linear-gradient(
        135deg,
        #0f2f59,
        #173b6c
    ) !important;

    transform: translateY(-1px);
}


/* Home primary button */

.primary-btn div.stButton > button {
    background: linear-gradient(
        135deg,
        #173b6c,
        #24548d
    ) !important;

    color: #ffffff !important;

    border: none !important;
}


/* ==========================================================
   SUCCESS / INFO
   ========================================================== */

.success-card {
    background: #ecfdf5;
    border: 2px solid #34d399;
    border-radius: 12px;

    padding: 12px;

    margin-top: 10px;

    text-align: center;
}

.success-title {
    color: #166534;
    font-size: 13px;
    font-weight: 900;
}

.success-text {
    color: #15803d;
    font-size: 11px;
    font-weight: 700;
    margin-top: 3px;
}


/* ==========================================================
   RESULT PANEL
   ========================================================== */

.result-panel {
    background: #ffffff;

    border: 1px solid #d5e1ee;

    border-radius: 16px;

    padding: 15px;

    margin-top: 13px;

    box-shadow:
        0 8px 24px rgba(23,59,108,.07);
}

.result-title {
    color: #173b6c;

    font-size: 16px;

    font-weight: 900;

    margin-bottom: 10px;

    display: flex;
    align-items: center;
    gap: 7px;
}

.result-image-box {
    background: #f8fafc;

    border: 1px solid #dbe5ef;

    border-radius: 12px;

    padding: 8px;

    text-align: center;

    margin-bottom: 10px;
}


/* ==========================================================
   STATUS
   ========================================================== */

.status-overflow {
    background: #fff1f2;

    border: 2px solid #f87171;

    border-radius: 13px;

    padding: 14px;

    color: #991b1b;

    margin-top: 10px;
}

.status-normal {
    background: #f0fdf4;

    border: 2px solid #4ade80;

    border-radius: 13px;

    padding: 14px;

    color: #166534;

    margin-top: 10px;
}

.status-no-detection {
    background: #fffbeb;

    border: 2px solid #fbbf24;

    border-radius: 13px;

    padding: 14px;

    color: #92400e;

    margin-top: 10px;
}

.status-title {
    font-size: 17px;

    font-weight: 900;

    margin-bottom: 7px;
}


/* ==========================================================
   DETECTION DETAILS
   ========================================================== */

.details-card {
    background: #f8fafc;

    border: 1px solid #dbe5ef;

    border-radius: 13px;

    padding: 13px 15px;

    margin-top: 10px;
}

.details-title {
    color: #173b6c;

    font-size: 13px;

    font-weight: 900;

    margin-bottom: 6px;
}

.detail-row {
    color: #334155;

    font-size: 12px;

    padding: 6px 0;

    border-bottom: 1px solid #e2e8f0;
}

.detail-row:last-child {
    border-bottom: none;
}


/* ==========================================================
   EMAIL ALERT
   ========================================================== */

.email-success {
    background: #dcfce7;

    border: 2px solid #22c55e;

    border-radius: 13px;

    padding: 13px;

    text-align: center;

    margin-top: 10px;
}

.email-success-title {
    color: #14532d;

    font-size: 14px;

    font-weight: 900;
}

.email-success-text {
    color: #166534;

    font-size: 11px;

    font-weight: 700;

    margin-top: 4px;
}


/* ==========================================================
   VIDEO
   ========================================================== */

.video-output-card {
    background: #f8fafc;

    border: 1px solid #dbe5ef;

    border-radius: 13px;

    padding: 9px;

    margin-bottom: 10px;
}


/* ==========================================================
   STREAMLIT MEDIA
   ========================================================== */

[data-testid="stImage"] img {
    border-radius: 11px !important;
    max-height: 360px !important;
    object-fit: contain !important;
}

[data-testid="stVideo"] {
    border-radius: 11px !important;
    overflow: hidden !important;
}


/* ==========================================================
   PROGRESS
   ========================================================== */

.stProgress > div > div > div > div {
    background: #2f80ed;
}


/* ==========================================================
   MOBILE
   ========================================================== */

@media (max-width: 900px) {

    .hero-title {
        font-size: 25px;
    }

    .hero-card,
    .description-card {
        min-height: auto;
    }

    .hero-career {
        font-size: 23px;
    }

    .io-card {
        min-height: auto;
        margin-bottom: 12px;
    }

}

</style>
""",
    unsafe_allow_html=True
)


# ============================================================
# HELPERS
# ============================================================

def show_logo(width=72):
    logo_path = os.path.join(
        os.path.dirname(__file__),
        "logo.png"
    )

    if os.path.exists(logo_path):
        st.image(
            logo_path,
            width=width
        )


# ============================================================
# LOAD MODEL
# ============================================================

@st.cache_resource
def load_model():

    model_path = os.path.join(
        os.path.dirname(__file__),
        "best.pt"
    )

    if not os.path.exists(model_path):
        raise FileNotFoundError(
            "best.pt file not found in the same folder as app.py"
        )

    return YOLO(model_path)


# ============================================================
# REVERSE GEOCODING
# ============================================================

@st.cache_data(ttl=300)
def reverse_geocode(latitude, longitude):

    try:

        response = requests.get(
            "https://nominatim.openstreetmap.org/reverse",

            params={
                "lat": latitude,
                "lon": longitude,
                "format": "jsonv2",
                "addressdetails": 1,
                "zoom": 18,
                "accept-language": "en",
            },

            headers={
                "User-Agent":
                "EcoBin-AI-Garbage-Overflow-Detection/1.0"
            },

            timeout=10,
        )

        response.raise_for_status()

        data = response.json()

        address = data.get(
            "address",
            {}
        )

        area = (
            address.get("suburb")
            or address.get("neighbourhood")
            or address.get("village")
            or address.get("town")
            or address.get("city")
            or address.get("municipality")
        )

        city = (
            address.get("city")
            or address.get("town")
            or address.get("village")
            or address.get("municipality")
        )

        state = address.get(
            "state",
            ""
        )

        postcode = address.get(
            "postcode",
            ""
        )

        parts = []

        if area:
            parts.append(area)

        if city and city != area:
            parts.append(city)

        if state:
            parts.append(state)

        if postcode:
            parts.append(postcode)

        if parts:
            return ", ".join(parts)

        return data.get(
            "display_name",
            "Area name unavailable"
        )

    except Exception:

        return "Area name unavailable"


# ============================================================
# LIVE LOCATION
# ============================================================

def get_live_location():

    location = streamlit_geolocation()

    if not location:
        return

    latitude = location.get(
        "latitude"
    )

    longitude = location.get(
        "longitude"
    )

    accuracy = location.get(
        "accuracy"
    )

    if latitude is None or longitude is None:
        return

    try:

        latitude = float(latitude)

        longitude = float(longitude)

        st.session_state.live_latitude = latitude
        st.session_state.live_longitude = longitude
        st.session_state.live_accuracy = accuracy

        st.session_state.live_area = reverse_geocode(
            latitude,
            longitude
        )

        st.session_state.location_loaded = True

    except Exception:

        st.session_state.location_loaded = False


# ============================================================
# LOCATION TEXT
# ============================================================

def get_location():

    area = st.session_state.live_area

    latitude = st.session_state.live_latitude

    longitude = st.session_state.live_longitude

    if (
        area
        and latitude is not None
        and longitude is not None
    ):

        return (
            f"{area}\n"
            f"Latitude: {latitude:.6f}\n"
            f"Longitude: {longitude:.6f}"
        )

    return "Live location not available"


# ============================================================
# MAP URL
# ============================================================

def get_map_url():

    latitude = st.session_state.live_latitude

    longitude = st.session_state.live_longitude

    if latitude is None or longitude is None:
        return None

    return (
        "https://www.google.com/maps/search/?api=1"
        f"&query={latitude},{longitude}"
    )


# ============================================================
# CURRENT TIME
# ============================================================

def get_current_time():

    return datetime.now(
        ZoneInfo("Asia/Kolkata")
    ).strftime(
        "%d-%b-%Y %I:%M:%S %p"
    )


# ============================================================
# LOCATION DISPLAY
# ============================================================

def display_live_location():

    st.markdown(
        """
<div class="location-card">

<div class="location-heading">
📍 Live Location
</div>
""",
        unsafe_allow_html=True
    )

    get_live_location()

    if st.session_state.location_loaded:

        area = st.session_state.live_area

        latitude = st.session_state.live_latitude

        longitude = st.session_state.live_longitude

        accuracy = st.session_state.live_accuracy

        accuracy_text = "Available"

        if accuracy is not None:

            try:

                accuracy_text = (
                    f"{float(accuracy):.1f} meters"
                )

            except Exception:

                accuracy_text = str(
                    accuracy
                )

        st.markdown(
            f"""
<div class="location-line">

<b>Current Area:</b> {area}<br>

<b>Latitude:</b> {latitude:.6f}
&nbsp;&nbsp;&nbsp;

<b>Longitude:</b> {longitude:.6f}<br>

<b>GPS Accuracy:</b> {accuracy_text}<br>

<b>Location Time:</b> {get_current_time()}

</div>
""",
            unsafe_allow_html=True
        )

        map_url = get_map_url()

        if map_url:

            st.link_button(
                "🗺️ Open Live Location in Google Maps",
                map_url,
                use_container_width=True
            )

    else:

        st.markdown(
            """
<div class="location-line">

📍 <b>Allow browser location permission</b>
to load your live area.

</div>
""",
            unsafe_allow_html=True
        )

    st.markdown(
        "</div>",
        unsafe_allow_html=True
    )


# ============================================================
# EMAIL ALERT
# ============================================================

def send_email_alert():

    try:

        sender_email = st.secrets[
            "EMAIL_SENDER"
        ]

        sender_password = st.secrets[
            "EMAIL_PASSWORD"
        ]

        receiver_email = st.secrets[
            "EMAIL_RECEIVER"
        ]

        message = EmailMessage()

        message["Subject"] = (
            "EcoBin AI - Garbage Overflow Alert"
        )

        message["From"] = sender_email

        message["To"] = receiver_email

        message.set_content(
            f"""
GARBAGE OVERFLOW DETECTED!

EcoBin AI
Smart Garbage Overflow Detection System

Live Location:
{get_location()}

Google Maps:
{get_map_url() or "Location unavailable"}

Date & Time:
{get_current_time()}

Status:
Violation Detected

Detection Class:
Overflow

This alert was automatically generated by EcoBin AI.
"""
        )

        with smtplib.SMTP(
            "smtp.gmail.com",
            587
        ) as server:

            server.starttls()

            server.login(
                sender_email,
                sender_password
            )

            server.send_message(
                message
            )

        return True

    except Exception as e:

        st.error(
            f"❌ Email alert failed: {e}"
        )

        return False


# ============================================================
# GENERATE ALERT
# ============================================================

def generate_alert():

    current_dt = datetime.now(
        ZoneInfo("Asia/Kolkata")
    )

    previous = (
        st.session_state.last_alert_time
    )

    if previous is not None:

        difference = (
            current_dt - previous
        ).total_seconds()

        if difference < 300:
            return

    if send_email_alert():

        st.session_state.last_alert_time = (
            current_dt
        )

        st.markdown(
            """
<div class="email-success">

<div class="email-success-title">
📧 ALERT EMAIL SENT SUCCESSFULLY
</div>

<div class="email-success-text">
The garbage overflow alert was sent to
the configured email address.
</div>

</div>
""",
            unsafe_allow_html=True
        )


# ============================================================
# NORMALIZE CLASS
# ============================================================

def normalize_class_name(name):

    return (
        str(name)
        .lower()
        .strip()
        .replace("_", " ")
        .replace("-", " ")
    )


# ============================================================
# EXTRACT DETECTIONS
# ============================================================

def extract_detections(result):

    detections = []

    if result.boxes is None:
        return detections

    for box in result.boxes:

        class_id = int(
            box.cls[0]
        )

        confidence = float(
            box.conf[0]
        )

        class_name = normalize_class_name(
            result.names[class_id]
        )

        detections.append(
            {
                "class": class_name,
                "confidence": confidence
            }
        )

    return detections


# ============================================================
# FINAL PREDICTION
# ============================================================

def get_final_prediction(detections):

    overflow_detections = [
        d
        for d in detections
        if d["class"] in [
            "overflow",
            "overclass",
            "garbage overflow",
            "overflowing",
            "overflowed"
        ]
        and d["confidence"] >= 0.30
    ]

    normal_detections = [
        d
        for d in detections
        if d["class"] in [
            "normal",
            "normal bin",
            "non overflow",
            "non-overflow"
        ]
    ]

    if overflow_detections:

        best = max(
            overflow_detections,
            key=lambda x: x["confidence"]
        )

        return (
            "GARBAGE OVERFLOW",
            best["confidence"]
        )

    if normal_detections:

        best = max(
            normal_detections,
            key=lambda x: x["confidence"]
        )

        return (
            "NORMAL",
            best["confidence"]
        )

    return (
        "NO CLEAR DETECTION",
        0.0
    )


# ============================================================
# IMAGE PREDICTION
# ============================================================

def predict_image(
    image,
    model
):

    image_np = np.array(
        image
    )

    results = model.predict(
        source=image_np,
        conf=0.20,
        verbose=False
    )

    result = results[0]

    detections = extract_detections(
        result
    )

    return (
        result,
        detections
    )


# ============================================================
# EMPTY MESSAGE
# ============================================================

def empty_box(
    heading,
    subtext,
    icon="📌"
):

    st.markdown(
        f"""
<div class="empty-message">

<div>

<div class="empty-icon">
{icon}
</div>

<div class="empty-heading">
{heading}
</div>

<div class="empty-sub">
{subtext}
</div>

</div>

</div>
""",
        unsafe_allow_html=True
    )


# ============================================================
# SECTION TITLE
# ============================================================

def section_title(
    icon,
    title
):

    st.markdown(
        f"""
<div class="mode-title">
{icon} {title}
</div>
""",
        unsafe_allow_html=True
    )


# ============================================================
# IMAGE RESULT
# ============================================================

def show_prediction_result(
    result,
    detections,
    title="Image Prediction Result"
):

    st.markdown(
        f"""
<div class="result-panel">

<div class="result-title">
🎯 {title}
</div>
""",
        unsafe_allow_html=True
    )

    # --------------------------------------------------------
    # ANNOTATED OUTPUT IMAGE
    # --------------------------------------------------------

    annotated = result.plot()

    st.markdown(
        '<div class="result-image-box">',
        unsafe_allow_html=True
    )

    # SAFE IMAGE DISPLAY
    st.image(
        annotated,
        width=650
    )

    st.markdown(
        "</div>",
        unsafe_allow_html=True
    )

    # --------------------------------------------------------
    # STATUS
    # --------------------------------------------------------

    status, confidence = get_final_prediction(
        detections
    )

    location_text = (
        get_location()
        .replace(
            "\n",
            " | "
        )
    )

    if status == "GARBAGE OVERFLOW":

        st.markdown(
            f"""
<div class="status-overflow">

<div class="status-title">
🚨 Garbage Overflow Detected
</div>

<b>Detection:</b> Overflow<br>

<b>Confidence:</b>
{confidence * 100:.2f}%<br>

<b>Location:</b>
{location_text}<br>

<b>Date & Time:</b>
{get_current_time()}<br>

<b>Status:</b>
Violation Detected

</div>
""",
            unsafe_allow_html=True
        )

        generate_alert()

    elif status == "NORMAL":

        st.markdown(
            f"""
<div class="status-normal">

<div class="status-title">
✅ No Garbage Overflow Detected
</div>

<b>Detection:</b> Normal<br>

<b>Confidence:</b>
{confidence * 100:.2f}%<br>

<b>Location:</b>
{location_text}<br>

<b>Date & Time:</b>
{get_current_time()}<br>

<b>Status:</b>
Normal

</div>
""",
            unsafe_allow_html=True
        )

    else:

        st.markdown(
            """
<div class="status-no-detection">

<div class="status-title">
⚠️ No Clear Detection
</div>

Please try another garbage image
with a clearer view of the bin.

</div>
""",
            unsafe_allow_html=True
        )

    # --------------------------------------------------------
    # DETECTION DETAILS
    # --------------------------------------------------------

    if detections:

        st.markdown(
            """
<div class="details-card">

<div class="details-title">
📊 Detection Details
</div>
""",
            unsafe_allow_html=True
        )

        for detection in detections:

            st.markdown(
                f"""
<div class="detail-row">

<b>
{detection["class"].title()}
</b>

&nbsp; — &nbsp;

{detection["confidence"] * 100:.2f}%
confidence

</div>
""",
                unsafe_allow_html=True
            )

        st.markdown(
            "</div>",
            unsafe_allow_html=True
        )

    else:

        st.markdown(
            """
<div class="details-card">

<div class="details-title">
📊 Detection Details
</div>

<div class="detail-row">
No objects detected above the confidence threshold.
</div>

</div>
""",
            unsafe_allow_html=True
        )

    st.markdown(
        "</div>",
        unsafe_allow_html=True
    )


# ============================================================
# VIDEO PROCESSING
# ============================================================

def process_video(
    video_path,
    model
):

    cap = cv2.VideoCapture(
        video_path
    )

    if not cap.isOpened():

        return (
            "NO CLEAR DETECTION",
            None,
            0.0
        )

    fps = cap.get(
        cv2.CAP_PROP_FPS
    )

    if fps <= 0:
        fps = 25

    width = int(
        cap.get(
            cv2.CAP_PROP_FRAME_WIDTH
        )
    )

    height = int(
        cap.get(
            cv2.CAP_PROP_FRAME_HEIGHT
        )
    )

    total_frames = int(
        cap.get(
            cv2.CAP_PROP_FRAME_COUNT
        )
    )

    raw_output = tempfile.NamedTemporaryFile(
        delete=False,
        suffix=".mp4"
    )

    raw_output_path = raw_output.name

    raw_output.close()

    h264_output = tempfile.NamedTemporaryFile(
        delete=False,
        suffix=".mp4"
    )

    h264_output_path = h264_output.name

    h264_output.close()

    writer = cv2.VideoWriter(
        raw_output_path,
        cv2.VideoWriter_fourcc(
            *"mp4v"
        ),
        fps,
        (
            width,
            height
        )
    )

    frame_number = 0

    overflow_count = 0

    normal_count = 0

    detection_count = 0

    best_overflow_confidence = 0.0

    progress = st.progress(
        0
    )

    while True:

        ret, frame = cap.read()

        if not ret:
            break

        results = model.predict(
            source=frame,
            conf=0.20,
            verbose=False
        )

        result = results[0]

        detections = extract_detections(
            result
        )

        status, confidence = (
            get_final_prediction(
                detections
            )
        )

        if status == "GARBAGE OVERFLOW":

            overflow_count += 1

            detection_count += 1

            best_overflow_confidence = max(
                best_overflow_confidence,
                confidence
            )

        elif status == "NORMAL":

            normal_count += 1

            detection_count += 1

        annotated_frame = result.plot()

        if status == "GARBAGE OVERFLOW":

            label = (
                "GARBAGE OVERFLOW DETECTED"
            )

        elif status == "NORMAL":

            label = "NORMAL"

        else:

            label = (
                "NO CLEAR DETECTION"
            )

        cv2.rectangle(
            annotated_frame,
            (10, 10),
            (560, 72),
            (25, 55, 95),
            -1
        )

        cv2.putText(
            annotated_frame,
            label,
            (25, 50),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.72,
            (255, 255, 255),
            2,
            cv2.LINE_AA
        )

        if confidence > 0:

            cv2.putText(
                annotated_frame,
                (
                    f"Confidence: "
                    f"{confidence * 100:.2f}%"
                ),
                (15, height - 18),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.62,
                (255, 255, 255),
                2,
                cv2.LINE_AA
            )

        writer.write(
            annotated_frame
        )

        frame_number += 1

        if total_frames > 0:

            progress.progress(
                min(
                    frame_number /
                    total_frames,
                    1.0
                )
            )

    cap.release()

    writer.release()

    progress.empty()

    # --------------------------------------------------------
    # FFMPEG CONVERSION
    # --------------------------------------------------------

    try:

        ffmpeg_path = (
            imageio_ffmpeg
            .get_ffmpeg_exe()
        )

        command = [
            ffmpeg_path,
            "-y",
            "-i",
            raw_output_path,
            "-c:v",
            "libx264",
            "-preset",
            "fast",
            "-crf",
            "23",
            "-pix_fmt",
            "yuv420p",
            "-movflags",
            "+faststart",
            h264_output_path
        ]

        subprocess.run(
            command,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            check=True
        )

        final_video_path = (
            h264_output_path
        )

    except Exception:

        final_video_path = (
            raw_output_path
        )

    # --------------------------------------------------------
    # READ VIDEO
    # --------------------------------------------------------

    try:

        with open(
            final_video_path,
            "rb"
        ) as video_file:

            output_video_bytes = (
                video_file.read()
            )

    except Exception:

        output_video_bytes = None

    # --------------------------------------------------------
    # CLEANUP
    # --------------------------------------------------------

    for path in [
        raw_output_path,
        h264_output_path
    ]:

        try:

            if os.path.exists(path):

                os.remove(path)

        except Exception:
            pass

    # --------------------------------------------------------
    # FINAL STATUS
    # --------------------------------------------------------

    if detection_count == 0:

        return (
            "NO CLEAR DETECTION",
            output_video_bytes,
            0.0
        )

    if overflow_count > 0:

        return (
            "GARBAGE OVERFLOW",
            output_video_bytes,
            best_overflow_confidence
        )

    return (
        "NORMAL",
        output_video_bytes,
        0.0
    )


# ============================================================
# HOME PAGE
# ============================================================

if st.session_state.page == "home":

    # --------------------------------------------------------
    # TITLE
    # --------------------------------------------------------

    st.markdown(
        '<div class="hero-title">♻️ EcoBin AI</div>',
        unsafe_allow_html=True
    )

    st.markdown(
        """
<div class="hero-subtitle">

Smart Garbage Overflow Detection
using Artificial Intelligence

</div>
""",
        unsafe_allow_html=True
    )

    # --------------------------------------------------------
    # HERO
    # --------------------------------------------------------

    hero_left, hero_right = st.columns(
        [1, 1.25],
        gap="large"
    )

    with hero_left:

        logo_path = os.path.join(
            os.path.dirname(__file__),
            "logo.png"
        )

        st.markdown(
            """
<div class="hero-card">

<div class="hero-left">

<div class="program-pill">
🎓 AICW PROGRAM
</div>

<div class="hero-career">
AI Career for Women
</div>

<div class="hero-capstone">
Capstone Project
</div>

<div class="hero-project-name">
♻️ EcoBin AI
</div>

</div>

</div>
""",
            unsafe_allow_html=True
        )

        if os.path.exists(
            logo_path
        ):

            st.markdown(
                """
<div style="
margin-top:-100px;
margin-left:25px;
position:relative;
z-index:2;
">
""",
                unsafe_allow_html=True
            )

            st.image(
                logo_path,
                width=64
            )

            st.markdown(
                "</div>",
                unsafe_allow_html=True
            )

    with hero_right:

        st.markdown(
            """
<div class="description-card">

<div class="description-heading">
🌱 Project Description
</div>

<p>
EcoBin AI is an AI-powered Smart Garbage Overflow Detection System
designed to automatically identify overflowing garbage bins using
computer vision and YOLOv8 object detection.
</p>

<p>
The system analyzes images, camera-captured photos, and CCTV/video
files to identify garbage overflow conditions.
</p>

<p>
The trained YOLOv8 model classifies the detected garbage condition
into two classes: <b>Normal</b> and <b>Overflow</b>.
</p>

<p>
When an overflow condition is detected, EcoBin AI automatically
generates an alert containing the live location, date and time,
and violation status. The alert is also sent to the configured
user's email address.
</p>

</div>
""",
            unsafe_allow_html=True
        )

    # --------------------------------------------------------
    # CAPABILITIES
    # --------------------------------------------------------

    st.markdown(
        """
<div class="section-title">
⚡ System Capabilities
</div>
""",
        unsafe_allow_html=True
    )

    caps = [
        (
            "🖼️",
            "Image Detection",
            "Analyze uploaded garbage images"
        ),
        (
            "📷",
            "Camera Detection",
            "Capture and detect using camera"
        ),
        (
            "🎥",
            "CCTV Analysis",
            "Process recorded video footage"
        ),
        (
            "🚨",
            "Smart Alerts",
            "Location and email notification"
        ),
    ]

    cap_cols = st.columns(
        4,
        gap="medium"
    )

    for col, (
        icon,
        title,
        text
    ) in zip(
        cap_cols,
        caps
    ):

        with col:

            st.markdown(
                f"""
<div class="capability-card">

<div class="capability-icon">
{icon}
</div>

<div class="capability-title">
{title}
</div>

<div class="capability-text">
{text}
</div>

</div>
""",
                unsafe_allow_html=True
            )

    st.markdown(
        '<div style="height:14px"></div>',
        unsafe_allow_html=True
    )

    # --------------------------------------------------------
    # START BUTTON
    # --------------------------------------------------------

    st.markdown(
        '<div class="primary-btn">',
        unsafe_allow_html=True
    )

    if st.button(
        "🔎  START AI DETECTION",
        key="start_detection",
        use_container_width=True
    ):

        st.session_state.page = (
            "predict"
        )

        st.rerun()

    st.markdown(
        "</div>",
        unsafe_allow_html=True
    )

    # --------------------------------------------------------
    # TEAM
    # --------------------------------------------------------

    st.markdown(
        """
<div class="section-title">
👥 Project Team
</div>
""",
        unsafe_allow_html=True
    )

    team_col, mail_col, guide_col = (
        st.columns(
            [1.25, 1.25, .85],
            gap="medium"
        )
    )

    with team_col:

        st.markdown(
            """
<div class="team-card">

<div class="team-heading">
TEAM MEMBERS
</div>

<div class="team-text">

1. K.Lalitha Devi<br>
2. Y.Haasini<br>
3. G.Sri Divya<br>
4. N.Sushma sri

</div>

</div>
""",
            unsafe_allow_html=True
        )

    with mail_col:

        st.markdown(
            """
<div class="team-card">

<div class="team-heading">
GMAIL
</div>

<div class="team-text">

<a href="mailto:lalithadevi825@gmail.com">
lalithadevi825@gmail.com
</a><br>

<a href="mailto:haasiniyanamadala@gmail.com">
haasiniyanamadala@gmail.com
</a><br>

<a href="mailto:galidivya534@gmail.com">
galidivya534@gmail.com
</a><br>

<a href="mailto:nadimpallisushmasri29@gmail.com">
nadimpallisushmasri29@gmail.com
</a>

</div>

</div>
""",
            unsafe_allow_html=True
        )

    with guide_col:

        st.markdown(
            """
<div class="team-card">

<div class="team-heading">
GUIDE NAME
</div>

<div class="team-text">

<b>MD. Abdul Aziz</b>

<br><br>

Trainer, Co-Lead-AICW

</div>

</div>
""",
            unsafe_allow_html=True
        )

    st.markdown(
        """
<div class="footer-text">

EcoBin AI • Smart Garbage Overflow Detection

</div>
""",
        unsafe_allow_html=True
    )


# ============================================================
# PREDICTION PAGE
# ============================================================

else:

    # --------------------------------------------------------
    # TOP
    # --------------------------------------------------------

    top_left, top_right = st.columns(
        [.2, 1],
        gap="medium"
    )

    with top_left:

        if st.button(
            "← Home",
            key="back_home"
        ):

            st.session_state.page = (
                "home"
            )

            st.rerun()

    with top_right:

        st.markdown(
            """
<div class="page-header">

<div class="page-header-title">
♻️ EcoBin AI Detection Center
</div>

<div class="page-header-sub">
AI-powered image, camera and CCTV
garbage overflow analysis
</div>

</div>
""",
            unsafe_allow_html=True
        )

    # --------------------------------------------------------
    # LOCATION
    # --------------------------------------------------------

    display_live_location()

    # --------------------------------------------------------
    # MODEL
    # --------------------------------------------------------

    try:

        model = load_model()

    except Exception as e:

        st.error(
            f"❌ best.pt model load avvaledu: {e}"
        )

        st.info(
            "Make sure best.pt is in the same folder as app.py."
        )

        st.stop()


    # ========================================================
    # IMAGE DETECTION
    # ========================================================

    section_title(
        "🖼️",
        "Image Detection"
    )

    image_upload_col, image_input_col, image_output_col = (
        st.columns(
            3,
            gap="medium"
        )
    )

    input_image = None

    uploaded_image = None

    # --------------------------------------------------------
    # UPLOAD
    # --------------------------------------------------------

    with image_upload_col:

        st.markdown(
            """
<div class="io-card">

<div class="io-card-title">
📤 Upload
</div>

<div class="io-card-subtitle">
Upload a garbage image for AI analysis
</div>
""",
            unsafe_allow_html=True
        )

        uploaded_image = st.file_uploader(
            "Choose an image",
            type=[
                "jpg",
                "jpeg",
                "png"
            ],
            key="image_upload"
        )

        if uploaded_image:

            # Clear previous result when a NEW image is uploaded
            if (
                st.session_state.image_file_name
                != uploaded_image.name
            ):

                st.session_state.image_result = None

                st.session_state.image_file_name = (
                    uploaded_image.name
                )

            st.markdown(
                """
<div class="success-card">

<div class="success-title">
✅ Image uploaded successfully
</div>

<div class="success-text">
Ready for AI detection
</div>

</div>
""",
                unsafe_allow_html=True
            )

        else:

            st.markdown(
                """
<div style="
margin-top:14px;
color:#475569;
font-size:12px;
font-weight:700;
text-align:center;
">
📁 Select a JPG, JPEG or PNG image
</div>
""",
                unsafe_allow_html=True
            )

        st.markdown(
            "</div>",
            unsafe_allow_html=True
        )


    # --------------------------------------------------------
    # INPUT
    # --------------------------------------------------------

    with image_input_col:

        st.markdown(
            """
<div class="io-card">

<div class="io-card-title">
🖼️ Input
</div>

<div class="io-card-subtitle">
Selected image preview
</div>
""",
            unsafe_allow_html=True
        )

        if uploaded_image:

            try:

                image_bytes = (
                    uploaded_image.getvalue()
                )

                input_image = Image.open(
                    BytesIO(
                        image_bytes
                    )
                ).convert(
                    "RGB"
                )

                # SAFE DISPLAY
                st.image(
                    np.array(
                        input_image
                    ),
                    width=360
                )

            except Exception as e:

                input_image = None

                st.error(
                    f"❌ Unable to load image: {e}"
                )

        else:

            empty_box(
                "No Image Selected",
                "Upload a garbage image to preview it here.",
                "🖼️"
            )

        st.markdown(
            "</div>",
            unsafe_allow_html=True
        )


    # --------------------------------------------------------
    # OUTPUT
    # --------------------------------------------------------

    with image_output_col:

        st.markdown(
            """
<div class="io-card">

<div class="io-card-title">
🎯 Output
</div>

<div class="io-card-subtitle">
AI detection result
</div>
""",
            unsafe_allow_html=True
        )

        if (
            uploaded_image
            and input_image is not None
        ):

            st.markdown(
                '<div class="detect-button">',
                unsafe_allow_html=True
            )

            detect_clicked = st.button(
                "🔍  DETECT IMAGE",
                key="detect_image",
                use_container_width=True
            )

            st.markdown(
                "</div>",
                unsafe_allow_html=True
            )

            if detect_clicked:

                with st.spinner(
                    "🤖 AI is analyzing the image..."
                ):

                    result, detections = (
                        predict_image(
                            input_image,
                            model
                        )
                    )

                st.session_state.image_result = (
                    result,
                    detections
                )

            # ------------------------------------------------
            # RESULT INSIDE OUTPUT CARD
            # ------------------------------------------------

            if (
                st.session_state.image_result
                is not None
            ):

                st.markdown(
                    """
<div class="success-card">

<div class="success-title">
✅ Prediction Completed
</div>

<div class="success-text">
AI detection result generated below
</div>

</div>
""",
                    unsafe_allow_html=True
                )

                result, detections = (
                    st.session_state.image_result
                )

                # SMALL OUTPUT IMAGE
                annotated = result.plot()

                st.markdown(
                    '<div class="result-image-box">',
                    unsafe_allow_html=True
                )

                st.image(
                    annotated,
                    width=360
                )

                st.markdown(
                    "</div>",
                    unsafe_allow_html=True
                )

                status, confidence = (
                    get_final_prediction(
                        detections
                    )
                )

                location_text = (
                    get_location()
                    .replace(
                        "\n",
                        " | "
                    )
                )

                # --------------------------------------------
                # STATUS
                # --------------------------------------------

                if status == "GARBAGE OVERFLOW":

                    st.markdown(
                        f"""
<div class="status-overflow">

<div class="status-title">
🚨 Garbage Overflow Detected
</div>

<b>Detection:</b> Overflow<br>

<b>Confidence:</b>
{confidence * 100:.2f}%<br>

<b>Location:</b>
{location_text}<br>

<b>Date & Time:</b>
{get_current_time()}<br>

<b>Status:</b>
Violation Detected

</div>
""",
                        unsafe_allow_html=True
                    )

                    generate_alert()

                elif status == "NORMAL":

                    st.markdown(
                        f"""
<div class="status-normal">

<div class="status-title">
✅ No Garbage Overflow Detected
</div>

<b>Detection:</b> Normal<br>

<b>Confidence:</b>
{confidence * 100:.2f}%<br>

<b>Location:</b>
{location_text}<br>

<b>Date & Time:</b>
{get_current_time()}<br>

<b>Status:</b>
Normal

</div>
""",
                        unsafe_allow_html=True
                    )

                else:

                    st.markdown(
                        """
<div class="status-no-detection">

<div class="status-title">
⚠️ No Clear Detection
</div>

Please try another image
with a clearer garbage-bin view.

</div>
""",
                        unsafe_allow_html=True
                    )

                # --------------------------------------------
                # DETECTION DETAILS
                # --------------------------------------------

                st.markdown(
                    """
<div class="details-card">

<div class="details-title">
📊 Detection Details
</div>
""",
                    unsafe_allow_html=True
                )

                if detections:

                    for detection in detections:

                        st.markdown(
                            f"""
<div class="detail-row">

<b>
{detection["class"].title()}
</b>

&nbsp; — &nbsp;

{detection["confidence"] * 100:.2f}%
confidence

</div>
""",
                            unsafe_allow_html=True
                        )

                else:

                    st.markdown(
                        """
<div class="detail-row">
No objects detected above threshold.
</div>
""",
                        unsafe_allow_html=True
                    )

                st.markdown(
                    "</div>",
                    unsafe_allow_html=True
                )

        else:

            empty_box(
                "Output Not Ready",
                "Upload an image and click Detect Image.",
                "🎯"
            )

        st.markdown(
            "</div>",
            unsafe_allow_html=True
        )


    # ========================================================
    # CAMERA
    # ========================================================

    section_title(
        "📷",
        "Camera Detection"
    )

    camera_col, camera_input_col, camera_output_col = (
        st.columns(
            3,
            gap="medium"
        )
    )

    camera_pil = None

    camera_image = None

    # --------------------------------------------------------
    # CAMERA
    # --------------------------------------------------------

    with camera_col:

        st.markdown(
            """
<div class="io-card">

<div class="io-card-title">
📷 Camera
</div>

<div class="io-card-subtitle">
Capture a garbage-bin image
</div>
""",
            unsafe_allow_html=True
        )

        camera_image = st.camera_input(
            "Capture image",
            key="camera"
        )

        if not camera_image:

            st.markdown(
                """
<div style="
text-align:center;
color:#475569;
font-size:12px;
font-weight:700;
margin-top:10px;
">
📷 Camera is ready
</div>
""",
                unsafe_allow_html=True
            )

        st.markdown(
            "</div>",
            unsafe_allow_html=True
        )


    # --------------------------------------------------------
    # CAMERA INPUT
    # --------------------------------------------------------

    with camera_input_col:

        st.markdown(
            """
<div class="io-card">

<div class="io-card-title">
🖼️ Input
</div>

<div class="io-card-subtitle">
Captured image preview
</div>
""",
            unsafe_allow_html=True
        )

        if camera_image:

            try:

                camera_bytes = (
                    camera_image.getvalue()
                )

                camera_pil = Image.open(
                    BytesIO(
                        camera_bytes
                    )
                ).convert(
                    "RGB"
                )

                st.image(
                    np.array(
                        camera_pil
                    ),
                    width=360
                )

            except Exception as e:

                camera_pil = None

                st.error(
                    f"❌ Unable to load camera image: {e}"
                )

        else:

            empty_box(
                "No Camera Image",
                "Capture an image using the camera.",
                "📷"
            )

        st.markdown(
            "</div>",
            unsafe_allow_html=True
        )


    # --------------------------------------------------------
    # CAMERA OUTPUT
    # --------------------------------------------------------

    with camera_output_col:

        st.markdown(
            """
<div class="io-card">

<div class="io-card-title">
🎯 Output
</div>

<div class="io-card-subtitle">
AI camera detection result
</div>
""",
            unsafe_allow_html=True
        )

        if (
            camera_image
            and camera_pil is not None
        ):

            st.markdown(
                '<div class="detect-button">',
                unsafe_allow_html=True
            )

            camera_clicked = st.button(
                "🔍  DETECT CAMERA IMAGE",
                key="detect_camera",
                use_container_width=True
            )

            st.markdown(
                "</div>",
                unsafe_allow_html=True
            )

            if camera_clicked:

                with st.spinner(
                    "🤖 Analyzing camera image..."
                ):

                    result, detections = (
                        predict_image(
                            camera_pil,
                            model
                        )
                    )

                st.session_state.camera_result = (
                    result,
                    detections
                )

            if (
                st.session_state.camera_result
                is not None
            ):

                st.markdown(
                    """
<div class="success-card">

<div class="success-title">
✅ Prediction Completed
</div>

<div class="success-text">
Camera detection result generated below
</div>

</div>
""",
                    unsafe_allow_html=True
                )

                result, detections = (
                    st.session_state.camera_result
                )

                annotated = result.plot()

                st.markdown(
                    '<div class="result-image-box">',
                    unsafe_allow_html=True
                )

                st.image(
                    annotated,
                    width=360
                )

                st.markdown(
                    "</div>",
                    unsafe_allow_html=True
                )

                status, confidence = (
                    get_final_prediction(
                        detections
                    )
                )

                location_text = (
                    get_location()
                    .replace(
                        "\n",
                        " | "
                    )
                )

                if status == "GARBAGE OVERFLOW":

                    st.markdown(
                        f"""
<div class="status-overflow">

<div class="status-title">
🚨 Garbage Overflow Detected
</div>

<b>Detection:</b> Overflow<br>

<b>Confidence:</b>
{confidence * 100:.2f}%<br>

<b>Location:</b>
{location_text}<br>

<b>Date & Time:</b>
{get_current_time()}<br>

<b>Status:</b>
Violation Detected

</div>
""",
                        unsafe_allow_html=True
                    )

                    generate_alert()

                elif status == "NORMAL":

                    st.markdown(
                        f"""
<div class="status-normal">

<div class="status-title">
✅ No Garbage Overflow Detected
</div>

<b>Detection:</b> Normal<br>

<b>Confidence:</b>
{confidence * 100:.2f}%<br>

<b>Location:</b>
{location_text}<br>

<b>Date & Time:</b>
{get_current_time()}<br>

<b>Status:</b>
Normal

</div>
""",
                        unsafe_allow_html=True
                    )

                else:

                    st.markdown(
                        """
<div class="status-no-detection">

<div class="status-title">
⚠️ No Clear Detection
</div>

Please try another image.

</div>
""",
                        unsafe_allow_html=True
                    )

                st.markdown(
                    """
<div class="details-card">

<div class="details-title">
📊 Detection Details
</div>
""",
                    unsafe_allow_html=True
                )

                if detections:

                    for detection in detections:

                        st.markdown(
                            f"""
<div class="detail-row">

<b>
{detection["class"].title()}
</b>

&nbsp; — &nbsp;

{detection["confidence"] * 100:.2f}%
confidence

</div>
""",
                            unsafe_allow_html=True
                        )

                else:

                    st.markdown(
                        """
<div class="detail-row">
No objects detected above threshold.
</div>
""",
                        unsafe_allow_html=True
                    )

                st.markdown(
                    "</div>",
                    unsafe_allow_html=True
                )

        else:

            empty_box(
                "Camera Output Not Ready",
                "Capture an image and click Detect Camera Image.",
                "🎯"
            )

        st.markdown(
            "</div>",
            unsafe_allow_html=True
        )


    # ========================================================
    # CCTV / VIDEO
    # ========================================================

    section_title(
        "🎥",
        "CCTV / Video Detection"
    )

    video_upload_col, video_input_col, video_output_col = (
        st.columns(
            3,
            gap="medium"
        )
    )

    uploaded_video = None

    # --------------------------------------------------------
    # VIDEO UPLOAD
    # --------------------------------------------------------

    with video_upload_col:

        st.markdown(
            """
<div class="io-card">

<div class="io-card-title">
📤 Upload Video
</div>

<div class="io-card-subtitle">
Upload CCTV footage for AI analysis
</div>
""",
            unsafe_allow_html=True
        )

        uploaded_video = st.file_uploader(
            "Choose CCTV/video",
            type=[
                "mp4",
                "avi",
                "mov",
                "mkv",
                "mpeg"
            ],
            key="video_upload"
        )

        if uploaded_video:

            if (
                st.session_state.video_file_name
                != uploaded_video.name
            ):

                st.session_state.video_result = None

                st.session_state.video_file_name = (
                    uploaded_video.name
                )

            st.markdown(
                """
<div class="success-card">

<div class="success-title">
✅ Video uploaded successfully
</div>

<div class="success-text">
Ready for CCTV analysis
</div>

</div>
""",
                unsafe_allow_html=True
            )

        else:

            empty_box(
                "No Video Selected",
                "Upload MP4, AVI, MOV, MKV or MPEG footage.",
                "🎥"
            )

        st.markdown(
            "</div>",
            unsafe_allow_html=True
        )


    # --------------------------------------------------------
    # VIDEO INPUT
    # --------------------------------------------------------

    with video_input_col:

        st.markdown(
            """
<div class="io-card">

<div class="io-card-title">
🎬 Input
</div>

<div class="io-card-subtitle">
Original CCTV footage
</div>
""",
            unsafe_allow_html=True
        )

        if uploaded_video:

            st.video(
                uploaded_video
            )

        else:

            empty_box(
                "CCTV Video Preview",
                "Your uploaded video will appear here.",
                "🎬"
            )

        st.markdown(
            "</div>",
            unsafe_allow_html=True
        )


    # --------------------------------------------------------
    # VIDEO OUTPUT
    # --------------------------------------------------------

    with video_output_col:

        st.markdown(
            """
<div class="io-card">

<div class="io-card-title">
🎯 Output
</div>

<div class="io-card-subtitle">
AI processed CCTV result
</div>
""",
            unsafe_allow_html=True
        )

        if uploaded_video:

            st.markdown(
                '<div class="detect-button">',
                unsafe_allow_html=True
            )

            video_clicked = st.button(
                "🎥  ANALYZE CCTV VIDEO",
                key="detect_video",
                use_container_width=True
            )

            st.markdown(
                "</div>",
                unsafe_allow_html=True
            )

            if video_clicked:

                temp_video = (
                    tempfile.NamedTemporaryFile(
                        delete=False,
                        suffix=".mp4"
                    )
                )

                temp_video.write(
                    uploaded_video.getvalue()
                )

                temp_video.close()

                try:

                    with st.spinner(
                        "🤖 AI is analyzing CCTV video..."
                    ):

                        video_result = process_video(
                            temp_video.name,
                            model
                        )

                    st.session_state.video_result = (
                        video_result
                    )

                except Exception as e:

                    st.error(
                        f"❌ Video processing failed: {e}"
                    )

                finally:

                    if os.path.exists(
                        temp_video.name
                    ):

                        os.remove(
                            temp_video.name
                        )

            # ------------------------------------------------
            # VIDEO RESULT INSIDE OUTPUT
            # ------------------------------------------------

            if (
                st.session_state.video_result
                is not None
            ):

                st.markdown(
                    """
<div class="success-card">

<div class="success-title">
✅ Video Analysis Completed
</div>

<div class="success-text">
AI processed video generated below
</div>

</div>
""",
                    unsafe_allow_html=True
                )

                (
                    video_status,
                    output_video_bytes,
                    video_conf
                ) = (
                    st.session_state.video_result
                )

                if output_video_bytes:

                    st.markdown(
                        '<div class="video-output-card">',
                        unsafe_allow_html=True
                    )

                    st.video(
                        output_video_bytes
                    )

                    st.markdown(
                        "</div>",
                        unsafe_allow_html=True
                    )

                else:

                    st.error(
                        "❌ Output video could not be generated."
                    )

                # --------------------------------------------
                # VIDEO STATUS
                # --------------------------------------------

                location_text = (
                    get_location()
                    .replace(
                        "\n",
                        " | "
                    )
                )

                if (
                    video_status
                    == "GARBAGE OVERFLOW"
                ):

                    st.markdown(
                        f"""
<div class="status-overflow">

<div class="status-title">
🚨 Garbage Overflow Detected
</div>

<b>Detection:</b> Overflow<br>

<b>Best Confidence:</b>
{video_conf * 100:.2f}%<br>

<b>Location:</b>
{location_text}<br>

<b>Date & Time:</b>
{get_current_time()}<br>

<b>Status:</b>
Violation Detected

</div>
""",
                        unsafe_allow_html=True
                    )

                    generate_alert()

                elif (
                    video_status
                    == "NORMAL"
                ):

                    st.markdown(
                        f"""
<div class="status-normal">

<div class="status-title">
✅ No Garbage Overflow Detected
</div>

<b>Detection:</b>
Normal<br>

<b>Location:</b>
{location_text}<br>

<b>Date & Time:</b>
{get_current_time()}<br>

<b>Status:</b>
Normal

</div>
""",
                        unsafe_allow_html=True
                    )

                else:

                    st.markdown(
                        """
<div class="status-no-detection">

<div class="status-title">
⚠️ No Clear Detection
</div>

No clear garbage condition
was detected in the video.

</div>
""",
                        unsafe_allow_html=True
                    )

                # --------------------------------------------
                # VIDEO DETAILS
                # --------------------------------------------

                st.markdown(
                    """
<div class="details-card">

<div class="details-title">
📊 Video Detection Details
</div>

<div class="detail-row">

<b>Analysis Type:</b>
CCTV / Video Analysis

</div>

<div class="detail-row">

<b>Final Status:</b>
""",
                    unsafe_allow_html=True
                )

                if (
                    video_status
                    == "GARBAGE OVERFLOW"
                ):

                    st.markdown(
                        """
<span style="
color:#b91c1c;
font-weight:900;
">
Garbage Overflow
</span>

</div>
""",
                        unsafe_allow_html=True
                    )

                elif (
                    video_status
                    == "NORMAL"
                ):

                    st.markdown(
                        """
<span style="
color:#15803d;
font-weight:900;
">
Normal
</span>

</div>
""",
                        unsafe_allow_html=True
                    )

                else:

                    st.markdown(
                        """
<span style="
color:#92400e;
font-weight:900;
">
No Clear Detection
</span>

</div>
""",
                        unsafe_allow_html=True
                    )

                st.markdown(
                    "</div>",
                    unsafe_allow_html=True
                )

        else:

            empty_box(
                "Video Output Not Ready",
                "Upload CCTV footage and click Analyze CCTV Video.",
                "🎯"
            )

        st.markdown(
            "</div>",
            unsafe_allow_html=True
        )


    # ========================================================
    # FOOTER
    # ========================================================

    st.markdown(
        """
<div class="footer-text">

EcoBin AI •
AI-Powered Smart Garbage Overflow Detection

</div>
""",
        unsafe_allow_html=True
    )
